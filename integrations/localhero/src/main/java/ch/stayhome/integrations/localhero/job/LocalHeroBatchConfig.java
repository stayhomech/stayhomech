...
public class LocalHeroBatchConfig {
    ...
    @SneakyThrows
    public PagingQueryMap queryMap(String className) {
        Class<?> clazz = PagingQueryMap.class.getClassLoader()
                .loadClass(className);
        return (PagingQueryMap) clazz.getDeclaredConstructor().newInstance();
    }
    ...
    private LocalHeroPostReader reader(LocalHeroChApi localHeroChApi) {
        return new LocalHeroPostReader(
                localHeroProperties.getPageSize(),
                localHeroChApi,
                queryMap(localHeroProperties.getQueryMapClass())
        );
    }
